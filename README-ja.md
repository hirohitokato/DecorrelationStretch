...
このツールは、HTML、JavaScript、Canvas、インラインWeb Workerだけで構成されています。サーバー、Python、WebAssembly、OpenCV.js、外部CDN、ビルド工程は不要です。

> **重要**
>
> このアプリは、原画像から派生した強調表示を作るためのものです。
> 補正後画像は判読補助用であり、原画像の代替証拠ではありません。
> 必ず原画像と比較して判断してください。
...
- モダンブラウザ上でローカル実行できます。
- 外部ライブラリに依存しません。
- クリックまたはドラッグアンドドロップで画像を読み込めます。
- 原画像と補正後画像を横並びで比較できます。
- 原画像上で矩形範囲を選択できます。
- Decorrelation Stretchを画像全体または選択した矩形領域に適用できます。
- Web Workerで処理するため、処理中もUIが完全には固まりにくい構造です。
...
ブラウザによっては `file://` 直開き時に一部機能が制限される場合があります。その場合は、同じフォルダで以下を実行してください。
...
最近のChrome、Edge、Firefox、Safariでの利用を想定しています。

## クイックスタート

1. `decorrelation-stretch.html` を開きます。
...
| 領域 | 役割 |
|---|---|
| 操作パネル | プリセット、数値パラメータ、実行・リセット・保存ボタン |
| 原画像ペイン | 入力画像の表示、矩形選択 |
| 補正後ペイン | 処理結果の表示、PNGドラッグ保存 |
| 処理ログ | 進捗、パラメータ、メタデータの表示 |
